// Palmer Penguins dataset (Horst, Hill & Gorman 2020; CC0). 344 rows ×
// 8 columns including naturally-occurring missing values — useful for
// stress-testing aggregators across realistic group sizes and the
// engine null-handling rules (numeric "" coerces past sum/avg/min/max,
// distinctCount skips empties, etc.).
//
// Columns:
...
//
// Total NAs: 2 each in bill_length_mm / bill_depth_mm / flipper_length_mm /
// body_mass_g (same rows), 11 in sex. year is always present.
...
function numField(key: keyof Penguin): AnyFieldDef {
	return {
		key: key as string,
		label: key as string,
		type: 'number',
		getValue: (row: Penguin) => {
			const v = row[key];
...
function strField(key: keyof Penguin): AnyFieldDef {
	return {
		key: key as string,
		label: key as string,
		type: 'string',
...
export const penguinFields: AnyFieldDef[] = [
	strField('species'),
	strField('island'),
	numField('bill_length_mm'),
	numField('bill_depth_mm'),
	numField('flipper_length_mm'),
	numField('body_mass_g'),
	strField('sex'),
	numField('year'),
];
