...
- Sort runs after filter and before take in a typical pipeline. queriton has an internal optimisation (`topK`) that fuses `.sort().take(n)` into one pass — covered in chapter 12.
